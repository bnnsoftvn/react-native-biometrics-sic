...
import com.rnbiometrics.security.util.DerInputStream;
...
public class RDN {
    ...
    final com.rnbiometrics.security.x509.AVA[] assertion;

    // cached immutable List of the AVAs
    private volatile List<com.rnbiometrics.security.x509.AVA> avaList;
    ...
    public RDN(String name, Map<String, String> keywordMap) throws IOException {
        ...
        int searchOffset = 0;
        int avaOffset = 0;
        List<com.rnbiometrics.security.x509.AVA> avaVec = new ArrayList<com.rnbiometrics.security.x509.AVA>(3);
        ...
        while (nextPlus >= 0) {
            ...
            if (nextPlus > 0 && name.charAt(nextPlus - 1) != '\\'
                ...
                com.rnbiometrics.security.x509.AVA ava = new com.rnbiometrics.security.x509.AVA(new StringReader(avaString), keywordMap);
                avaVec.add(ava);
        ...
        assertion = avaVec.toArray(new com.rnbiometrics.security.x509.AVA[avaVec.size()]);
    }
    ...
    RDN(String name, String format, Map<String, String> keywordMap)
        ...
        while (nextPlus >= 0) {
            ...
            if (nextPlus > 0 && name.charAt(nextPlus - 1) != '\\' ) {
                ...
                // Parse AVA, and store it in vector
                com.rnbiometrics.security.x509.AVA ava = new com.rnbiometrics.security.x509.AVA
                    (new StringReader(avaString), com.rnbiometrics.security.x509.AVA.RFC2253, keywordMap);
                avaVec.add(ava);
        ...
        com.rnbiometrics.security.x509.AVA ava = new com.rnbiometrics.security.x509.AVA(new StringReader(avaString), com.rnbiometrics.security.x509.AVA.RFC2253, keywordMap);
        avaVec.add(ava);
     ...
     * @throws IOException on parsing error.
     */
    RDN(com.rnbiometrics.security.util.DerValue rdn) throws IOException {
        if (rdn.tag != com.rnbiometrics.security.util.DerValue.tag_Set) {
            throw new IOException("X500 RDN");
        }
        DerInputStream dis = new com.rnbiometrics.security.util.DerInputStream(rdn.toByteArray());
        com.rnbiometrics.security.util.DerValue[] avaset = dis.getSet(5);

        assertion = new com.rnbiometrics.security.x509.AVA[avaset.length];
        for (int i = 0; i < avaset.length; i++) {
            assertion[i] = new com.rnbiometrics.security.x509.AVA(avaset[i]);
        }
    }
     ...
     */
    RDN(int i) { assertion = new com.rnbiometrics.security.x509.AVA[i]; }

    public RDN(com.rnbiometrics.security.x509.AVA ava) {
        if (ava == null) {
            throw new NullPointerException();
        }
        assertion = new com.rnbiometrics.security.x509.AVA[] { ava };
    }

    public RDN(com.rnbiometrics.security.x509.AVA[] avas) {
     ...
     */
    public List<com.rnbiometrics.security.x509.AVA> avas() {
        List<com.rnbiometrics.security.x509.AVA> list = avaList;
        if (list == null) {
     ...
     */
    com.rnbiometrics.security.util.DerValue findAttribute(com.rnbiometrics.security.util.ObjectIdentifier oid) {
        for (int i = 0; i < assertion.length; i++) {
     ...
     */
    void encode(com.rnbiometrics.security.util.DerOutputStream out) throws IOException {
        out.putOrderedSetOf(com.rnbiometrics.security.util.DerValue.tag_Set, assertion);
    }
    ...
    private String toRFC2253StringInternal
        ...
        if (!canonical) {
            ...
            List<com.rnbiometrics.security.x509.AVA> avaList = new ArrayList<com.rnbiometrics.security.x509.AVA>(assertion.length);
            for (int i = 0; i < assertion.length; i++) {
...
class AVAComparator implements Comparator<com.rnbiometrics.security.x509.AVA> {

    private static final Comparator<com.rnbiometrics.security.x509.AVA> INSTANCE = new AVAComparator();

    private AVAComparator() {
        // empty
    }

    static Comparator<com.rnbiometrics.security.x509.AVA> getInstance() {
        return INSTANCE;
    }
     ...
     */
    public int compare(com.rnbiometrics.security.x509.AVA a1, AVA a2) {
