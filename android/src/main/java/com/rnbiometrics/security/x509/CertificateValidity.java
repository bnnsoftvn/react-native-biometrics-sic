...
public class CertificateValidity implements CertAttrSet<String> {
    ...
    private void construct(com.rnbiometrics.security.util.DerValue derVal) throws IOException {
        ...
        DerInputStream derIn = new com.rnbiometrics.security.util.DerInputStream(derVal.toByteArray());
        com.rnbiometrics.security.util.DerValue[] seq = derIn.getSequence(2);
        if (seq.length != 2)
            throw new IOException("Invalid encoding for CertificateValidity");

        if (seq[0].tag == com.rnbiometrics.security.util.DerValue.tag_UtcTime) {
            notBefore = derVal.data.getUTCTime();
        } else if (seq[0].tag == com.rnbiometrics.security.util.DerValue.tag_GeneralizedTime) {
            notBefore = derVal.data.getGeneralizedTime();
        } else {
            throw new IOException("Invalid encoding for CertificateValidity");
        }

        if (seq[1].tag == com.rnbiometrics.security.util.DerValue.tag_UtcTime) {
            notAfter = derVal.data.getUTCTime();
        } else if (seq[1].tag == com.rnbiometrics.security.util.DerValue.tag_GeneralizedTime) {
     ...
     */
    public CertificateValidity(com.rnbiometrics.security.util.DerInputStream in) throws IOException {
        com.rnbiometrics.security.util.DerValue derVal = in.getDerValue();
        construct(derVal);
    }
    ...
    public void encode(OutputStream out) throws IOException {
        ...
        com.rnbiometrics.security.util.DerOutputStream pair = new com.rnbiometrics.security.util.DerOutputStream();
        ...
        seq.write(com.rnbiometrics.security.util.DerValue.tag_Sequence, pair);
