...
public class GeneralSubtrees implements Cloneable {
    ...
    private final List<com.rnbiometrics.security.x509.GeneralSubtree> trees;

    // Private variables
    private static final int NAME_DIFF_TYPE = com.rnbiometrics.security.x509.GeneralNameInterface.NAME_DIFF_TYPE;
    private static final int NAME_MATCH = com.rnbiometrics.security.x509.GeneralNameInterface.NAME_MATCH;
    private static final int NAME_NARROWS = com.rnbiometrics.security.x509.GeneralNameInterface.NAME_NARROWS;
    private static final int NAME_WIDENS = com.rnbiometrics.security.x509.GeneralNameInterface.NAME_WIDENS;
    private static final int NAME_SAME_TYPE = com.rnbiometrics.security.x509.GeneralNameInterface.NAME_SAME_TYPE;

    /**
     * The default constructor for the class.
     */
    public GeneralSubtrees() {
        trees = new ArrayList<com.rnbiometrics.security.x509.GeneralSubtree>();
    }

    private GeneralSubtrees(GeneralSubtrees source) {
        trees = new ArrayList<com.rnbiometrics.security.x509.GeneralSubtree>(source.trees);
    }
     ...
     */
    public GeneralSubtrees(com.rnbiometrics.security.util.DerValue val) throws IOException {
        this();
        ...
        while (val.data.available() != 0) {
            ...
            com.rnbiometrics.security.x509.GeneralSubtree tree = new com.rnbiometrics.security.x509.GeneralSubtree(opt);
            add(tree);
        }
    }

    public com.rnbiometrics.security.x509.GeneralSubtree get(int index) {
    ...
    public void add(com.rnbiometrics.security.x509.GeneralSubtree tree) {
        if (tree == null) {
    ...
    public boolean contains(com.rnbiometrics.security.x509.GeneralSubtree tree) {
        if (tree == null) {
            throw new NullPointerException();
        }
    ...
    public Iterator<com.rnbiometrics.security.x509.GeneralSubtree> iterator() {
        return trees.iterator();
    }

    public List<com.rnbiometrics.security.x509.GeneralSubtree> trees() {
        return trees;
    }
     ...
     */
    public void encode(com.rnbiometrics.security.util.DerOutputStream out) throws IOException {
     ...
     */
    private com.rnbiometrics.security.x509.GeneralNameInterface getGeneralNameInterface(int ndx) {
        return getGeneralNameInterface(get(ndx));
    }

    private static com.rnbiometrics.security.x509.GeneralNameInterface getGeneralNameInterface(com.rnbiometrics.security.x509.GeneralSubtree gs) {
        com.rnbiometrics.security.x509.GeneralName gn = gs.getName();
        com.rnbiometrics.security.x509.GeneralNameInterface gni = gn.getName();
        return gni;
    }
    ...
    private void minimize() {
        ...
        for (int i = 0; i < size(); i++) {
            com.rnbiometrics.security.x509.GeneralNameInterface current = getGeneralNameInterface(i);
            boolean remove1 = false;

            /* compare current to subsequent elements */
            for (int j = i + 1; j < size(); j++) {
                com.rnbiometrics.security.x509.GeneralNameInterface subsequent = getGeneralNameInterface(j);
                switch (current.constrains(subsequent)) {
                    case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_DIFF_TYPE:
                        /* not comparable; different name types; keep checking */
                        continue;
                    case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_MATCH:
                        /* delete one of the duplicates */
                        remove1 = true;
                        break;
                    case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_NARROWS:
                        /* subsequent narrows current */
                        /* remove narrower name (subsequent) */
                        remove(j);
                        j--; /* continue check with new subsequent */
                        continue;
                    case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_WIDENS:
                        /* subsequent widens current */
                        /* remove narrower name current */
                        remove1 = true;
                        break;
                    case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
     ...
     */
    private com.rnbiometrics.security.x509.GeneralSubtree createWidestSubtree(com.rnbiometrics.security.x509.GeneralNameInterface name) {
        try {
            com.rnbiometrics.security.x509.GeneralName newName;
            switch (name.getType()) {
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_ANY:
                // Create new OtherName with same OID as baseName, but
                // empty value
                com.rnbiometrics.security.util.ObjectIdentifier otherOID = ((com.rnbiometrics.security.x509.OtherName)name).getOID();
                newName = new com.rnbiometrics.security.x509.GeneralName(new OtherName(otherOID, null));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_RFC822:
                newName = new com.rnbiometrics.security.x509.GeneralName(new RFC822Name(""));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_DNS:
                newName = new com.rnbiometrics.security.x509.GeneralName(new DNSName(""));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_X400:
                newName = new com.rnbiometrics.security.x509.GeneralName(new X400Address((byte[])null));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_DIRECTORY:
                newName = new com.rnbiometrics.security.x509.GeneralName(new X500Name(""));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_EDI:
                newName = new com.rnbiometrics.security.x509.GeneralName(new EDIPartyName(""));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_URI:
                newName = new com.rnbiometrics.security.x509.GeneralName(new URIName(""));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_IP:
                newName = new com.rnbiometrics.security.x509.GeneralName(new IPAddressName((byte[])null));
                break;
            case com.rnbiometrics.security.x509.GeneralNameInterface.NAME_OID:
                newName = new GeneralName
                    (new OIDName(new com.rnbiometrics.security.util.ObjectIdentifier((int[])null)));
                break;
            default:
                throw new IOException
                    ("Unsupported GeneralNameInterface type: " + name.getType());
            }
            return new com.rnbiometrics.security.x509.GeneralSubtree(newName, 0, -1);
        } catch (IOException e) {
    ...
    public GeneralSubtrees intersect(GeneralSubtrees other) {
        ...
        // same type in this.
        for (int i = 0; i < size(); i++) {
            com.rnbiometrics.security.x509.GeneralNameInterface thisEntry = getGeneralNameInterface(i);
            ...
            for (int j = 0; j < other.size(); j++) {
                com.rnbiometrics.security.x509.GeneralSubtree otherEntryGS = other.get(j);
                com.rnbiometrics.security.x509.GeneralNameInterface otherEntry =
            ...
            if (sameType) {
                ...
                for (int j = 0; j < size(); j++) {
                    com.rnbiometrics.security.x509.GeneralNameInterface thisAltEntry = getGeneralNameInterface(j);

                    if (thisAltEntry.getType() == thisEntry.getType()) {
                        for (int k = 0; k < other.size(); k++) {
                            com.rnbiometrics.security.x509.GeneralNameInterface othAltEntry =
                ...
                if (intersection == false) {
                    ...
                    com.rnbiometrics.security.x509.GeneralSubtree widestSubtree =
        ...
        for (int i = 0; i < other.size(); i++) {
            ...
            com.rnbiometrics.security.x509.GeneralNameInterface otherEntry = getGeneralNameInterface(otherEntryGS);
            boolean diffType = false;
            for (int j = 0; j < size(); j++) {
                com.rnbiometrics.security.x509.GeneralNameInterface thisEntry = getGeneralNameInterface(j);
                switch (thisEntry.constrains(otherEntry)) {
                    case NAME_DIFF_TYPE:
                        diffType = true;
    ...
    public void reduce(GeneralSubtrees excluded) {
        ...
        for (int i = 0, n = excluded.size(); i < n; i++) {
            com.rnbiometrics.security.x509.GeneralNameInterface excludedName = excluded.getGeneralNameInterface(i);
            for (int j = 0; j < size(); j++) {
                com.rnbiometrics.security.x509.GeneralNameInterface permitted = getGeneralNameInterface(j);
