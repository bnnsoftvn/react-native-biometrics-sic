...
public class KeyIdentifier {
     ...
     */
    public KeyIdentifier(com.rnbiometrics.security.util.DerValue val) throws IOException {
    ...
    public KeyIdentifier(PublicKey pubKey)
        throws IOException
    {
        com.rnbiometrics.security.util.DerValue algAndKey = new DerValue(pubKey.getEncoded());
        if (algAndKey.tag != com.rnbiometrics.security.util.DerValue.tag_Sequence)
            throw new IOException("PublicKey value is not a valid "
                                  + "X.509 public key");

        com.rnbiometrics.security.x509.AlgorithmId algid = AlgorithmId.parse(algAndKey.data.getDerValue());
